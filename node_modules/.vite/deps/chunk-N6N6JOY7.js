...
} from "./chunk-MWCABCQ7.js";
import {
  luma
} from "./chunk-JNCR2PF3.js";
import {
  require_react
...
function L(n2, l3, u4, t3, i4, r3, o3, e3, f4, c3, a3) {
  ...
  for (f4 = T(u4, l3, m3, f4, b2), s3 = 0; s3 < b2; s3++) null != (p3 = u4.__k[s3]) && (h3 = -1 != p3.__i && m3[p3.__i] || d, p3.__i = s3, _3 = q(n2, p3, h3, i4, r3, o3, e3, f4, c3, a3), v3 = p3.__e, p3.ref && h3.ref != p3.ref && (h3.ref && J(h3.ref, null, p3), a3.push(p3.ref, p3.__c || v3, p3)), null == y3 && null != v3 && (y3 = v3), (g2 = !!(4 & p3.__u)) || h3.__k === p3.__k ? (f4 = j(p3, f4, n2, g2), g2 && h3.__e && (h3.__e = null)) : "function" == typeof p3.type && void 0 !== _3 ? f4 = _3 : v3 && (f4 = v3.nextSibling), p3.__u &= -7);
  return u4.__e = y3, f4;
}
...
function q(n2, u4, t3, i4, r3, o3, e3, f4, c3, a3) {
  var s3, h3, p3, v3, y3, d3, _3, k3, x2, M, $2, I2, P2, A3, H2, T3 = u4.type;
...
function G(u4, t3, i4, r3, o3, e3, f4, c3, a3) {
  var s3, h3, p3, v3, y3, w3, _3, m3 = i4.props || d, k3 = t3.props, x2 = t3.type;
...
function RangeInput(props) {
  const { className = "", min: min3, max: max3, step, value, orientation, pageSize, stepButtons = false, startButtonAriaLabel, endButtonAriaLabel, eventTarget, decorations = [], onChange } = props;
  ...
  const clampedStart = clamp3(value[0], min3, min3 + maxStart);
  const { thumbLength, thumbOffset } = T2(() => {
    ...
    const ratio = maxStart <= 0 ? 0 : clamp3((clampedStart - min3) / maxStart, 0, 1);
    return {
      thumbLength: Math.max(0, Math.min(nextThumbLength, 1)),
      thumbOffset: travel * ratio
    };
  }, [trackLength, range, rangeSize, maxStart, clampedStart, min3]);
  const emitRange = q2((nextStart) => {
    if (!onChange) {
      return;
    }
    const clamped = clamp3(nextStart, min3, min3 + maxStart);
    onChange([clamped, clamped + rangeSize]);
  }, [onChange, min3, maxStart, rangeSize]);
  ...
  const handleTrackClick = q2((event) => {
    ...
    emitRange(min3 + ratio * maxStart);
  }, [vertical, trackLength, thumbLength, emitRange, min3, maxStart]);
  ...
  const handleKeyDown = q2((event) => {
    switch (event.key) {
      ...
      case "Home":
        ...
        break;
      case "End":
        emitRange(min3 + maxStart);
        event.preventDefault();
        break;
      default:
        break;
    }
  }, [vertical, emitRange, clampedStart, step, range, pageSize, rangeSize, min3, maxStart]);
  ...
  const decorationElements = T2(() => {
    ...
    return decorations.map((decoration, index) => {
      ...
      const startRatio = (start - min3) / range;
      const endRatio = (end - min3) / range;
...
var TimelineWidget = class extends Widget {
  ...
  constructor(props = {}) {
    ...
    this.tick = () => {
      const { timeRange: [min3, max3], step, loop } = this.props;
      if (step > 0) {
  ...
  play() {
    this._playing = true;
    const { timeRange: [min3, max3] } = this.props;
    if (this.props.time === void 0 && this.getTime() >= max3) {
      this.currentTime = min3;
      this.props.onTimeChange(min3);
      this.props.timeline?.setTime(min3);
    }
    this.updateHTML();
    this.tick();
  }
...
//# sourceMappingURL=chunk-N6N6JOY7.js.map
