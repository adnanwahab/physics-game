...
const KeelingCurve = () => {
  ...
  useEffect(() => {
    ...
    const seasonalData = [
      { month: "Jan", val: -0.8 }, { month: "Apr", val: 2.8 }, 
      { month: "Jul", val: 0.5 }, { month: "Oct", val: -3.2 }, { month: "Dec", val: -0.2 }
    ];
    ...
    xAxis.selectAll("path, line").style("stroke", "#ccc").style("stroke-width", "1px");
    ...
    yAxis.selectAll("path, line").style("stroke", "#ccc").style("stroke-width", "1px");
    ...
    const insetX = d3.scaleLinear().domain([0, 11]).range([20, insetW - 20]);
    const insetY = d3.scaleLinear().domain([-4, 4]).range([insetH - 20, 20]);

    // Render Inset Internal Axes
    insetG.append("g")
      .attr("transform", `translate(0, ${insetY(0)})`)
      .call(d3.axisBottom(insetX).tickValues([0, 3, 6, 9, 11]).tickFormat((d, i) => ["Jan", "Apr", "Jul", "Oct", "Dec"][i]));
    ...
    // Draw Inset Curve
    const insetLineFn = d3.line().x(d => insetX(d.monthIdx)).y(d => insetY(d.val));
    insetG.append("path")
    ...
    box2.append("text").attr("x", 15).attr("y", 23).attr("fill", "#000").style("font-size", "12px").style("font-weight", "bold").text("This is what has");
    box2.append("text").attr("x", 15).attr("y", 41).attr("fill", "#000").style("font-size", "12px").style("font-weight", "bold").text("happened");
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '20px', fontFamily: 'Arial, sans-serif' }}>
      <svg 
        ref={svgRef} 
        width={850} 
        height={650} 
