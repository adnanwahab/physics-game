...
async function loadLevelCuboids(levelId, Jolt, bodyInterface, scene, dynamicObjects) {
    ...
    try {
        ...
        if (levelData && levelData.length > 0 && levelData[0].objects) {
            ...
            const redMaterial = new THREE.MeshStandardMaterial({ color: 0xff0000, metalness: 0.3, roughness: 0.7 });
            const cheeseMaterial = new THREE.MeshStandardMaterial({ color: 0xffd700, metalness: 0.5, roughness: 0.3, emissive: 0xffaa00, emissiveIntensity: 0.3 });
            const deskMaterial = new THREE.MeshStandardMaterial({ color: 0x8b4513, metalness: 0.1, roughness: 0.8 });
            const personMaterial = new THREE.MeshStandardMaterial({ color: 0x4a6fa5, metalness: 0.1, roughness: 0.7 });
            const LAYER_NON_MOVING = 0;

            for (let index = 0; index < objects.length; index++) {
                const obj = objects[index];
                const forcedRotation = (index === 2 && obj.type === 'cuboid') ? [0, Math.PI / 2, 0] : null;
                ...
                if (obj.type === 'desk') {
                    ...
                    createBox(Jolt, bodyInterface, async (body) => {
                        ...
                        try {
                            ...
                            desk.traverse((child) => { if (child.isMesh) { child.material = deskMaterial; child.geometry.translate(0, -obj.size[1] / 2, 0); } });
                            ...
                            const soundWaves = createSoundWaveRings({ position: new THREE.Vector3(0, obj.size[1] / 2 + 0.02, 0), maxRadius: 3.5, speed: 1.5 });
                            ...
                            person.traverse((child) => { if (child.isMesh) child.material = personMaterial; });
                            person.position.set(0, obj.size[1] / 2 - 0.75, -0.15);
                            desk.add(person);
                            scene.add(desk);
                        } catch (error) { console.error('Error loading desk model:', error); }
                    }, position, rotation, halfExtent, Jolt.EMotionType_Static, LAYER_NON_MOVING);
                    continue;
                }
                ...
                if (obj.type === 'cuboid' || obj.type === 'cheese') {
                    ...
                    if (forcedRotation) {
                        ...
                        rotation = new Jolt.Quat(q.x, q.y, q.z, q.w);
                    } else if (obj.rotation[0] === 0 && obj.rotation[1] === 0 && obj.rotation[2] === 0) {
                        rotation = Jolt.Quat.prototype.sIdentity();
                    } else {
                        const q = quatFromEulerArray(obj.rotation);
                        rotation = new Jolt.Quat(q.x, q.y, q.z, q.w);
                    }
                    if (obj.type === 'cheese') cheesePosition = new THREE.Vector3(obj.position[0], obj.position[1], obj.position[2]);
                    const material = obj.type === 'cheese' ? cheeseMaterial : redMaterial;
                    createBox(Jolt, bodyInterface, (body) => addToScene(body, Jolt, bodyInterface, scene, dynamicObjects, getThreeObjectForBody, material), position, rotation, halfExtent, Jolt.EMotionType_Static, LAYER_NON_MOVING);
                }
            }
        }
    } catch (error) {
